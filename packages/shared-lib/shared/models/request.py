...
class TargetInfo(BaseModel):
    ...
    domain: str | None = Field(
        None, description="Domain name, auto-extracted from URL if not provided"
    )
    web_type: WebType | None = Field(
        None, description="Detected web type, filled by Phase 0 analyzer"
    )
...
class DataSpec(BaseModel):
    ...
    required_fields: list[str] = Field(
        default_factory=list, description="Fields that must be present in output"
    )
    optional_fields: list[str] = Field(
        default_factory=list, description="Fields that are nice to have"
    )
...
class ScopeConfig(BaseModel):
    ...
    max_records: int = Field(
        default=10000, ge=1, le=1000000, description="Maximum records to collect"
    )
...
class OutputConfig(BaseModel):
    ...
    format: OutputFormat = Field(
        default=OutputFormat.JSON, description="Output data format"
    )
    destination: OutputDestination = Field(
        default=OutputDestination.DATA_LAKE, description="Storage destination"
    )
...
class ScheduleConfig(BaseModel):
    ...
    frequency: ScheduleFrequency = Field(
        default=ScheduleFrequency.ONE_TIME, description="Crawl frequency"
    )
...
class AuthConfig(BaseModel):
    ...
    login_url: str | None = Field(None, description="Login page URL")
    username_selector: str | None = Field(
        None, description="CSS selector for username field"
    )
    password_selector: str | None = Field(
        None, description="CSS selector for password field"
    )
    submit_selector: str | None = Field(
        None, description="CSS selector for submit button"
    )
    username: str | None = Field(None, description="Login username")
    password: str | None = Field(
        None, description="Login password (use env var reference)"
    )
...
class ConstraintsConfig(BaseModel):
    ...
    rate_limit_rps: float = Field(
        default=2.0, ge=0.1, le=100, description="Requests per second limit"
    )
    ...
    proxy_required: bool = Field(
        default=False, description="Whether approved network proxy routing is required"
    )
...
class CrawlRequest(BaseModel):
    ...
    schedule: ScheduleConfig = Field(
        default_factory=ScheduleConfig, description="Schedule configuration"
    )
    constraints: ConstraintsConfig = Field(
        default_factory=ConstraintsConfig, description="Crawl constraints"
    )
