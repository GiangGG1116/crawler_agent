...
class ValidationResult(BaseModel):
    ...
    def evaluate(self) -> bool:
        ...
        if self.critical_failed:
            self.is_valid = False
            self.decision_reason = f"CRITICAL checks failed: {', '.join(self.critical_failed)}"
...
class QualityReport(BaseModel):
    ...
    completeness_score: float = Field(0.0, ge=0, le=100, description="% records with all required fields")
    validity_score: float = Field(0.0, ge=0, le=100, description="% fields passing type validation")
    uniqueness_score: float = Field(0.0, ge=0, le=100, description="% unique records after dedup")
    freshness_score: float = Field(0.0, ge=0, le=100, description="Data freshness score")
    overall_score: float = Field(0.0, ge=0, le=100, description="Weighted average quality score")
...
class CrawlResult(BaseModel):
    ...
    template_id: Optional[str] = None
    template_failure: Optional[TemplateFailure] = None
